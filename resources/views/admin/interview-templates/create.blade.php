<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-gray-950">Tambah Template Interview</h2>
                <p class="mt-1 text-sm text-gray-500">Buat template interview baru beserta kategori dan aspek penilaiannya.</p>
            </div>
    ...
    <div class="py-6 sm:py-10">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm sm:p-6" x-data="templateForm()">
                <form method="POST" action="{{ route('admin.interview-templates.store') }}" class="space-y-6">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <x-input-label for="name" value="Nama Template" />
                            <x-text-input id="name" name="name" class="mt-1 block w-full" :value="old('name')" placeholder="Contoh: Form Interview Mekanik" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input type="number" id="min_recommended_percentage" name="min_recommended_percentage" class="mt-1 block w-full" :value="old('min_recommended_percentage', 70)" min="0" max="100" required />
                        ...
                        <div>
                            ...
                            <x-text-input type="number" id="min_considered_percentage" name="min_considered_percentage" class="mt-1 block w-full" :value="old('min_considered_percentage', 50)" min="0" max="100" required />
                            <x-input-error :messages="$errors->get('min_considered_percentage')" class="mt-2" />
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <input type="checkbox" id="is_active" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" checked>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-gray-200 pt-6">
                        ...
                        <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Simpan Template</button>
                    </div>
                </form>
            </div>
        </div>
